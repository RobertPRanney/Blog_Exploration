...
def predicted_value_plot(model, df, column, classification=False, class_pred='None', discrete_col=False, freq=True, scatter=True, scatter_num=None, fname=None, num_linspace=100, response_label=None, cmap=('#4c72b0', '#c44e52'), figsize=(8, 6), xlim=None):
    ...
    INPUT:
        ...
        xlim: tuple (int, int)
            min and max of x axis, if not given then min and max of data is used
    '''
    ...
    if response_label:
        ...
        ax1.set_ylabel('Predicted Probability ($\hat{\pi}$)', fontsize=18)
    else:
        ax1.set_ylabel('Predicted Response ($\hat{y}$)', fontsize=18)
    ax1.set_xlabel('{}'.format(column), fontsize=18)
    plt.title('Predicted Values Plot for {}'.format(column), fontsize=22)
