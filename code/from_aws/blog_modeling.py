...
from helpers import is_outlier


# Constant Section
RANDOM_NUM              = 42     # for reproducibility between files and runs
BLOG_DF_CATEGORIZED     = 'picked_dfs/blog_df_posts_categorized.pkl'
...
if __name__ == '__main__':
    ...
    # drop outliers, they are not helping
    blog_df = blog_df[~is_outlier(blog_df['subscribers_count'])]

    # pop off target column
    y = blog_df.pop('subscribers_count')

    # make topics dummies
    blog_df = pd.concat([blog_df, pd.get_dummies(blog_df['most_common_topic'])], axis=1)

    # save the final blog that was fit
    pickle.dump( blog_df, open( 'picked_dfs/final_blog_df.pkl', "wb" ) )
    pickle.dump( y, open( 'picked_dfs/final_y.pkl', 'wb' ) )
    ...
    # Pickle best model
    pickle.dump( best_est, open( 'blog_pickled_models/' + model_name + '_' + score_name + '.pkl', "wb" ) )

    # Apeend results to tx as redundant backup
    with open('blog_models_results.txt', 'a') as f:
